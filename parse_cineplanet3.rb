...
if ARGV[0]
  ...
  page.css('.bloque-peliculas-busqueda').each_with_index do |item, index|
    pelicula = item.css('div.cabecera strong').first.text.split.join(' ')
    puts pelicula
    ...
    horarios = item.css('div.contenido div.informacion p.texto').first.text.strip.gsub(/\s{2,}/, ", ")
    if horarios[0] != 'P'
      day = {date: Date.today.to_s.split('-'), times: horarios}
      days << day
      puts horarios
    end
  end
    #
  # page.css('#lista-pelicula div.cabecera strong').each_with_index do |item, index|
  #
  #   url2 = item[:href]
  #   retries = 42
  #   begin
