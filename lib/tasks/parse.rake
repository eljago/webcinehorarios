...
def fetch_page(url_str)
  begin
    ...
    if response.code == 200
      ...
      puts 'nil response. Bad URL?' if Rails.env.development?
      nil
    end
    ...
    shows.each do |show|
      ...
      if show.metacritic_url.present?
        ...
        if page
          ...
          if text.present? && text.to_i != 0
            puts "\t\tmeta: #{text.to_i}" if Rails.env.development?
      ...
      if show.imdb_code.present?
        ...
        if page
          ...
          unless score == 0
            puts "\t\timdb: #{score}" if Rails.env.development?
      ...
      if show.rotten_tomatoes_url.present?
        ...
        if page
          ...
          if span.present?
            ...
            if score != 0
              puts "\t\troten: #{score}" if Rails.env.development?
